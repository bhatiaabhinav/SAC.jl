...
mutable struct SACLearner{Tₛ<:AbstractFloat, Tₐ<:AbstractFloat} <: AbstractHook
    ...
    function SACLearner(π::SACPolicy{Tₛ, Tₐ}, critic, α, η_actor, η_critic; polyak=0.995, min_explore_steps=10000, train_interval=1, batch_size=32, buffer_size=1000000) where {Tₛ <: AbstractFloat, Tₐ <: AbstractFloat}
        buff = CircularBuffer{Tuple{Vector{Tₛ}, Vector{Tₐ}, Float64, Vector{Tₛ}, Bool}}(buffer_size)
        new{Tₛ, Tₐ}(π, (critic, deepcopy(critic)), α, polyak, min_explore_steps, train_interval, batch_size, nothing, buff, (deepcopy(critic), deepcopy(critic)), Adam(η_actor), Adam(η_critic))
    end
end
...
function poststep(sac::SACLearner{Tₛ, Tₐ}; env::AbstractMDP{Vector{Tₛ}, Vector{Tₐ}}, steps::Int, rng::AbstractRNG, kwargs...) where {Tₛ, Tₐ}
    @unpack π, critics, α, ρ, batch_size, s, critics′ = sac

    a, r, s′, d, γ = copy(action(env)), reward(env), copy(state(env)), in_absorbing_state(env), discount_factor(env)
    ...
    if steps >= sac.min_explore_steps && steps % sac.train_interval == 0
        ...
        α = clamp(exp(log(α) - 0.0001f0 * (H - target_ent)), 0.001f0, 1000f0)
        sac.α = α
        ...
        if steps % 1000 == 0
            v̄ = -mean(ℓθ)
            @info "learning stats" steps ℓϕ v̄ H α
        end
    end
    nothing
end
